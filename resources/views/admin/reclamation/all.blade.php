    ...
    <div class="card shadow mb-4">
      <div class="card-body">
        <div class="table-responsive">
            <form action="{{ route('reclamation.manyDelete') }}" method="POST" style="overflow: hidden">
                @csrf
                @method('DELETE')
                        ...
                        @foreach ($reclamations as $item)
                            <tr>
                                ...
                                <td class="text-center">
                                    ...
                                    @endswitch
                                    {{-- {{ $item->type == 'reclamation' ? '<span class="badge badge-warning">' . $item->type . '</span>' : '<span class="badge badge-info">' . $item->type . '</span>' }} --}}
                                </td>
                                <td class="text-center">
                                    {{ $item->fullName }}
                                </td>
                                ...
                                <td class="text-center">
                                    <a href="{{ route('reclamation.updateForm', ['id' => $item->id]) }}" class="btn btn-warning btn-circle btn-sm">
                                        <i class="fas fa-info-circle"></i>
                                    </a>
                                    <a href="{{ route('reclamation.delete', ['id' => $item->id]) }}" class="btn btn-danger btn-circle btn-sm ess-link-checked"
